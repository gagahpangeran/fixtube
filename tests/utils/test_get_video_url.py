from fixtube.utils import get_video_url
...
def test_get_video_url(video_info):
    url = get_video_url(video_info["formats"])
    assert url == VALID_VIDEO_URL


def test_get_video_url_format_no_vcodec(video_info):
    formats = video_info["formats"]
    for format in formats:
        if "vcodec" in format:
            format["vcodec"] = "none"

    url = get_video_url(formats)
    assert url is None


def test_get_video_url_format_no_acodec(video_info):
    formats = video_info["formats"]
    for format in formats:
        if "acodec" in format:
            format["acodec"] = "none"
